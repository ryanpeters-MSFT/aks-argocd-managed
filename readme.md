...
For this demo, Argo CD polls Git every 10 seconds with no reconciliation jitter. The reconciliation interval controls how often Argo CD checks Git for changes. Jitter adds a random delay to that interval to spread requests from multiple applications; `0s` disables that delay. The managed extension stores the live values as `timeout.reconciliation` and `timeout.reconciliation.jitter` in the `argocd-cm` ConfigMap in the `argocd` namespace. Use a longer interval, nonzero jitter, or webhooks for production to avoid unnecessary repository traffic.
